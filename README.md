...
![](docs/images/info.svg) To obtain your Customer Id and zAdviser Access
...
2.  Install the Topaz Workbench CLI on the Jenkins instances that will
    ...
    Workbench CLI installation instructions, please refer to the [Topaz Workbench Install Guide](https://docs.compuware.com/kb/KB2001/PDF/TopazWorkbench_Install.pdf).
...
![](docs/images/info.svg) If no host connection appears in the Host
...
-   **Access key** : Enter the zAdviser services access key provided by
    Compuware.
-   **Customer ID** : Enter your customer ID.
-   **Encryption key** : Enter an encryption key used to encrypt the
    zAdviser data that is uploaded to
    Compuware.
-   **Days in the past to start extracting data** : Enter the number of
...
![](docs/images/info.svg) Alternatively, to add a connection,
...
![](docs/images/info.svg) Alternatively, click **Add **to add credentials
using the Credentials Plugin. Refer to the Jenkins documentation for the
Credentials Plugin.

-   **JCL** : Update the JCL as appropriate for your
    organization:
...
to Compuware for hosting in Kibana.

![](docs/images/info.svg) This build step should be paired with a
...
![](docs/images/info.svg) To manage connections, click **Manage
...
-   **Sample Step** : Select one of the zAdviser build actions from
    above.
